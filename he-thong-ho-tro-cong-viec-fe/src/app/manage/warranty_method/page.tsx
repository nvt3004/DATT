...
import Add_WarrantyMethod_Dialog from "./addDialong";
import Edit_WarrantyMethod_Dialog from "./editDialong";
...
import { deleteWarrantyMethod, fetchWarrantyMethod } from "@/store/slices/warrantymethod/warrantymethodAction";
...
const arr = [
  ...
  { title: "Phương thức bảo hành", href: "/manage/warrantyMethod" },
];

export default function PagePage() {
  const dispatch = useDispatch<ThunkDispatch<any, any, any>>();
  // API tác nhân
  const { items, loading, error } = useSelector((state: any) => state.warrantyMethod);
  // handle pagination
  ...
  // handle active user
  const [activeUuser, setActiveUser] = useState<ActiveUser | any>(
    undefined
  );
  ...
  const handlerDelete = async (id: number) => {
    try {
      await dispatch(deleteWarrantyMethod(id));
  ...
  useEffect(() => {
    dispatch(fetchWarrantyMethod({ activePage, searchValue }));
  ...
  const rows = items?.result?.content?.map(
    (item: string | any, index: number) => (
      ...
      >
        ...
        <Tooltip label={item?.noiDung} position="top" withArrow multiline
