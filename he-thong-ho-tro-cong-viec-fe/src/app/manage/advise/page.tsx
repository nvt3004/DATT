...
interface ActiveValue {
    ...
    email: string;
    soDienThoai: string;
    danhXungTen: string | number;
}
const arr = [
    { title: "Trang chủ", href: "/manage" },
    { title: "Quản lý", href: "/manage" },
    { title: "Tư vấn", href: "/manage/advise" },
];
export default function PagePage() {
    ...
    const rows = items?.content?.map((item: string | any, index: number) => (
        ...
        >
            ...
            <Table.Td style={{ padding: "10px" }}>{item?.email}</Table.Td>
            <Table.Td style={{ padding: "10px" }}>{item?.soDienThoai}</Table.Td>
            <Table.Td
                style={{
