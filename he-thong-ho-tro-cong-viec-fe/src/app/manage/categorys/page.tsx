...
import Add_Hangmuc_Dialog from "./addDialong";
import Edit_Hangmuc_Dialog from "./editDialong";
...
interface ActiveUser {
    ...
    soLuong: number;
    moTa: string;
    donViTinh: string | number;
}
const arr = [
    { title: "Trang chủ", href: "/manage" },
    { title: "Quản lý", href: "/manage" },
    { title: "Hạng mục", href: "/manage/hangmuc" },
];
export default function PagePage() {
    const dispatch = useDispatch<ThunkDispatch<any, any, any>>();
    // API tác nhân
    const { items, loading, error } = useSelector(
        (state: any) => state.hangmuc
    );
    // handle pagination
    ...
    // handle active user
    const [activeUuser, setActiveUser] = useState<ActiveUser | any>(undefined);
    ...
    const rows = items?.result?.content?.map(
        (item: string | any, index: number) => (
            ...
            >
                ...
                <Table.Td style={{ padding: "10px" }}>{item.soLuong}</Table.Td>
                <Table.Td style={{ padding: "10px" }}>
                    <Tooltip
                        label={item?.moTa}
                        position="top"
