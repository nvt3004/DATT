...
import {
    ...
    EditProductSeverDetail,
    fetchProductSeverDetailById,
...
export default function PagePage({ params }: { params: { slug: string } }) {
    ...
    const sanphammaychuchitiet = useSelector((state: any) => state.productseverdetail);
    ...
    useEffect(() => {
        if (sanphammaychuchitiet?.items?.result?.thongSo?.id) {

            setSelectedRowsThongSo([sanphammaychuchitiet?.items?.result.thongSo.id]);
        }
        if (sanphammaychuchitiet?.items?.result?.mayChu?.id) {

            setSelectedRowsMayChu([sanphammaychuchitiet?.items?.result.mayChu.id]);
        }
        if (sanphammaychuchitiet?.items?.result?.thongSoGroup?.id) {

            setSelectedRowsNhomThongSo([sanphammaychuchitiet?.items?.result.thongSoGroup.id]);
        }
        if (sanphammaychuchitiet?.items?.result?.giaTri) {
            form.setValues({
                value: sanphammaychuchitiet?.items?.result?.giaTri,
            });
        }
    }, [sanphammaychuchitiet]);
    ...
    const handleSubmit = async (values: typeof form.values | any) => {
        ...
        try {
            const res = await dispatch(EditProductSeverDetail(data));
            setLoading(true);
            if (res.payload.code === 1000) {
                ...
                notifications.show({
                    ...
                    message: "Phần mềm máy chủ đã được chỉnh sửa.",
                    color: "teal",
                    autoClose: 3000,
                    position: "top-right",
                });
    ...
    useEffect(() => {
        ...
        dispatch(fetchProductSeverDetailById(id));
        ...
        <>
            ...
            <Box style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
                ...
                <Box className="py-2 px-2 bg-[#339af0] text-white rounded-md my-3">
                    <Input.Label size="17">Sửa phần mềm máy chủ</Input.Label>
                </Box>
                <Card
                    withBorder
                    shadow="md"
